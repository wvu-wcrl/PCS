/* File convolutional.h
   ...
   Copyright (C) 2006-2009, Matthew C. Valenti
...
/* Function Viterbi()
  ...
  Description: Uses the Viterbi algorithm to perform hard-decision decoding of a convolutional code.
	...
	Input parameters:
		...
		r[]			The received signal in LLR-form. 
		KK			The constraint length of the convolutional code.
...
{
    /* local variables */
	int i, t, state, states;
	int number_symbols, number_nodes;
	float metric;
	float max_val;
    
    /* static arrays */
    static float *prev_section, *next_section;
	static int *prev_bit;
	static int *prev_state;
	static float *metric_c;	/* Set of all possible branch metrics */
	static float *rec_array;   /* Received values for one trellis section */
    
    /* keep track of prior values, so we know if we need to reinitialize */
    static int last_nn = -1;
    static int last_states = -1;
    static int last_number_symbols = -1;
    static int last_number_nodes = -1;
	
	/* some derived constants */
	states = 1 << (KK-1);			/* 2^mm, where mm=KK-1 */
	number_symbols = 1 << nn;	    /* 2^nn */
    number_nodes = states*(LL+KK-1);  /* number nodes in trellis */
    ...
    if ( number_nodes != last_number_nodes ) {
        free( prev_bit );
        free( prev_state );      
        prev_bit = calloc( number_nodes, sizeof(int) );
        prev_state = calloc( number_nodes, sizeof(int) );
        last_number_nodes = number_nodes;
    }
...
{
    /* local variables */
	int i, t, state, states, number_nodes;
    int max_state = 0;
    ...
    number_nodes = states*(LL+depth);
    if ( number_nodes != last_number_nodes ) {
        #ifdef DEBUG
        mexPrintf( "Trellis size changed, reinitializing\n" );
        #endif
        free( prev_bit );
        free( prev_state );      
        prev_bit = calloc( number_nodes, sizeof(int) );
