...
public class AddUsers_ver1 extends Composite implements ClickHandler
{
	...
	public void onClick(ClickEvent event) 
	{
		...
		if(source == btnCreate)
		{
			...
			for(int i = 0; i < userCount; i++)
			{
				if(usernames[i].trim().length() > 0)
				{
					...
					if(tokens.length == 2)
					{
						...
						email = tokens[1].trim();	
						System.out.println("User: " + username + " username validity: " + username.matches(usernameRegex) + " email validity: " + email.matches(emailRegex));
			...
			{
				lblMessage.setText("Please enter a \"comma (,)\" separated valid username and email address for each user.");
			}
		}	
		if(source == btnAdd)
		{
	...
	{		
		...
		public void onSuccess(ArrayList<String> errorUsers)
		{
			...
			else
			{
				...
				lblMessage.setText("Error in creating following users in the list box. Please check if the username(s) and the corresponding email address is/are already taken."  );
				txtUsernames.setText(errorUsernames);
			}			
		}
	};
