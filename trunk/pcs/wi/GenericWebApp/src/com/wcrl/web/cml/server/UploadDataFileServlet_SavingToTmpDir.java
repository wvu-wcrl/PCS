...
import com.allen_sauer.gwt.log.client.Log;
 ...
 public class UploadDataFileServlet_SavingToTmpDir extends HttpServlet {
	 ...
	 public UploadDataFileServlet_SavingToTmpDir()
	 {
		 super();
	 }
    ...
    {   		 
        	 ...
        	 try
        	 {
        		 if(fileExists)
            	 {
                	 ...
                	 else
                	 {
                		 ...
                		 removeFile.delete();        
                		 createFile(uploadedFile, item, response, dir, login);  
                	 }            		 
            	 }
            	 else
            	 {
     ...
     public synchronized void createFile(File uploadedFile, FileItem item, HttpServletResponse response, String dir, String login)
     {
    	 try 
         {
    		 String tomcatWebAppDataTempDir = constants.getString("TomcatWebAppDataTempDir");
    		 String fileName = uploadedFile.getName();
    		 String tempDirPath = tomcatWebAppDataTempDir + fileName;
    		 File tempFileUploadDir = new File(tomcatWebAppDataTempDir);
    		 File tempFileUpload = new File(tempDirPath);
    		 System.out.println("tempDirPath: " + tempDirPath + " DestinationPath: " + dir);
    		 
    		 boolean success = false;    		  
             if(!tempFileUploadDir.exists())
             {
            	 success = tempFileUploadDir.mkdirs();
             }
             else
             {
            	 success = true;
    		 ...
    		 if(success)
    		 {
    			 if (tempFileUpload.createNewFile()) 
    			 {
    			     item.write(tempFileUpload);
    			     response.setStatus(HttpServletResponse.SC_CREATED);			     
    			     response.flushBuffer();
    			     
    			     FileIO fileIO = new FileIO();
    			     int exitCode = -1;
    			     exitCode = fileIO.callChangeOwnershipScript(login, tempDirPath);
    			     if(exitCode == 0)
    			     {
    			    	 exitCode = fileIO.callMoveFileScript(tempDirPath, dir);
    			    	 if(exitCode == 0)
    				     {
    				    	 String msg = 1 + "~Data file added.";					 
    					     Log.info("Message: " + msg + " Absolute path: " + uploadedFile.getAbsolutePath() + " Path: " + uploadedFile.getPath());			       
