...
public class GetJobDetails_bkp extends RemoteServiceServlet implements GetJobDetailsService
{
	/**
	 * 
	 */
	...
	public JobItem getJobDetails(JobItem item, int a)
	{	
	...
	public JobItem getJobData(File file, String fileName, String filePath, JobItem item)
	{
		...
		try
		{
			...
			System.out.println("MLStructure: " + mlStructure + " >>>>>>>" + mlStructure.getField("Results"));
			//Results
			...
			if(mlStructure.getField("Results") != null && mlStructure.getField("Results").getType() == 2)
			{
				...
				for(int i = 0; i < count; i++)
				{
					...
					if(text.contains(":"))
					{
						...
						String value = valueTokens[1].trim();
