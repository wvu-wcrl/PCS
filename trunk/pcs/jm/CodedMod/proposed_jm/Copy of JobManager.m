classdef JobManager < handle
    ...
    properties(SetAccess = protected)
        HomeRoot             % path to directory containing user home directories
        Check4NewUserPeriod  % how long to wait to check for new users to the system
        LogFileName          % file to log job manager output
        vqFlag               % verbose or quiet logging
        MaxTimes             % maximum number of recent trial numbers and processing times of each worker node saved for billing purposes
    end
    ...
    methods
        ...
        function obj = JobManager()
            % if no configuration file is passed to the constructor,  use
            % default settings
             if( nargin<1 || isempty(cfgRootIn) ), cfgRootIn = []; end    
            
            obj.JobManagerParam = ob.InitJobManager(cfgRootIn);
        ...
        function CodedModJobManager(cfgRootIn)
            ...
            TempDirResult = fullfile(obj.JobManagerParam.HomeRoot,'pcs','jm','CodedMod','Temp');
