classdef MeanOutageNakagamiExact < handle
    ...
    methods
        % constructor
        function obj = MeanOutageNakagamiExact( r_net ,r_ex,alpha,m,m_i,M,PG, varargin )
            ...
            for s=0:obj.m-1
                ...
                for r=0:s
                    ...
                    for ellset=1:size( obj.indices{r+1}, 1)
                        ...
                        sum_ell = sum_ell + prod( (repmat(p',1,obj.M).*repmat(factors*(2/(obj.alpha*obj.c_i^(2/obj.alpha)*(obj.r_net)^2)),length(p),1).*(1-(obj.r_ex/obj.r_net)^2)^-1 + repmat((1-p)',1,obj.M).*repmat((elli==0),length(p),1))') ;
                    end
