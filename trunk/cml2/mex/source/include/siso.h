/* File siso.h
   ...
   Functions in siso.h are part of the Iterative Solutions 
...
)
{
    ...
    int  number_nodes;      /* number nodes in trellis */
    float app_in;
    int mask;
    
    /* static arrays */
    static float *num_llr_c, *den_llr_c; /* Temp variable for LLR of code bits */
    static float *metric_c;	/* Set of all possible branch metrics */
    static float *beta;		/* Reverse trellis metrics for the entire trellis. */
    static float *alpha;		/* Forward trellis metric for the current stage only. */
    static float *alpha_prime;	/* Forward trellis metrics for the last stage only. */
    ...
    number_nodes = max_states*(LL+KK);  /* number nodes in trellis */
    
    /* dynamically allocate memory, if necessary */
    if ( max_states != last_states ) {
        #ifdef DEBUG
    ...
    if ( number_nodes != last_number_nodes ) {
        #ifdef DEBUG
        mexPrintf( "number nodes = %d\n", number_nodes );
        #endif
        free( beta );
        beta = calloc( number_nodes, sizeof(float) );
