...
{		
								...
								if(jobFile.isFile())
								{
									...
									{				
										...
										else if(statusDirectory.equals("JobRunning"))
										{
											...
											{
												status = "Running";
												jobId=getJobNumber(jobFile.getPath());
											}
											else
											{
										...
										{
											status = "Done";
											 MatchingScore=getScore(jobFile.getPath());
											 jobId=getJobNumber(jobFile.getPath());
										}
										...
										//item.setJobId(jobId);
										int cnt = fileItems.size();
										System.out.println("~~~~~~cnt: " + cnt + " user: " + user + " status: " + status);
										item.setId(cnt+1);
										fileItems.add(item);							
									}
								}
	...
	public String getScore(String filePath)
	{				
		MatFileReader matfilereader=null;
		String matchingscore = "Fetching the Score";
		try
		{
			matfilereader = new MatFileReader(filePath);
	...
	public int getJobNumber(String filePath)
	{
		MatFileReader matfilereader=null;
		int jobId = 0;
		try
