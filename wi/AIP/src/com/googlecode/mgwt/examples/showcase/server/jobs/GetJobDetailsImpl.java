...
public class GetJobDetailsImpl extends RemoteServiceServlet implements GetJobDetailsService
{
	/**
	 * 
	 */
	...
	//	dataFiles = getOutputFiles(dataFilesDir, fileName, dataFiles, path1 + File.separator + userProjectPath, "data");
		dataFiles = getDataFiles(dataFilesDir, fileName, dataFiles, path1 + File.separator + userProjectPath, "data",item);
		String[] fileData1 = new String[3];
		fileData1[0] = fileName;
		//fileData[1] = rootPathForDownload + path1 + File.separator + userProjectPath + dir + File.separator + fileName;
		fileData1[1] = path1 + File.separator + userProjectPath + constants.getString("JobOut") + File.separator + fileName;
		fileData1[2] = rootPath + constants.getString("JobOut") + File.separator + fileName;
		//System.out.println("Link File path: " + fileData[1]);
		//Log.info("Link File path: " + fileData[1]);
		dataFiles.put(1, fileData);
	...
	public Map<Integer, String[]> getDataFiles(String outputFilesDir, String jobOriginalFileName, Map<Integer, String[]> outputFiles, String rootPathForDownload, String from, JobItem item)
	{		
		File dirPath = new File(outputFilesDir);
		String rootPath = constants.getString("path");
		String projectConstant = constants.getString("projects");
		...
		String dir = constants.getString("JobOut");
		String userProjectPath = userName + File.separator + projectConstant + File.separator + "IrisCloud" + File.separator;
		rootPath = rootPath + userProjectPath;
		String filePath1 = rootPath + dir + File.separator + jobOriginalFileName;
		File aFile = new File(filePath1);
		if(aFile.exists())
		{
			try
			{
				
				matfilereader = new MatFileReader(filePath1);			
				
				if(matfilereader.getMLArray("JobParam") != null)
				{
					MLStructure mlStructure = (MLStructure) matfilereader.getMLArray("JobParam");
					if(mlStructure.getField("ImageOnePath") != null)
					{
						MLChar ImageOne = (MLChar) mlStructure.getField("ImageOnePath");
						String ImageOneStr = ImageOne.contentToString();
						
						if(ImageOneStr.length() > 0)
						{
							ImageOnePath = parseString(ImageOneStr);
							double statusValue = 0;
							try
							{
								statusValue = Double.parseDouble(ImageOnePath);
							}
							catch(NumberFormatException e)
							{
								System.out.println("Job " + item.getJobId() + " ImageOne Path not a double value.");
								Log.info("Job " + item.getJobId() + " ImageOne Path not a double value.");
								e.printStackTrace();
							}
						/*String[] tokens = ImageOnePath.split("\\.");
               			 int cnt = tokens.length;
               			 ImageOnePath=tokens[0]+ "." + tokens[cnt-1];*/
							ImageOnePath=ImageOnePath.substring((ImageOnePath.lastIndexOf(File.separator))+1);
							System.out.println("ImagOnePath" + ImageOnePath);
							Log.info("ImagOnePath" + ImageOnePath);
               			/*Path p=Paths.get(ageOnePath);
						ImageOnePath=p.getFileName().toString();*/
						}
					}
					if(mlStructure.getField("ImageTwoPath") != null)
					{
						MLChar ImageTwo = (MLChar) mlStructure.getField("ImageTwoPath");
						String ImageTwoStr = ImageTwo.contentToString();
						
						if(ImageTwoStr.length() > 0)
						{
							ImageTwoPath = parseString(ImageTwoStr);
							double statusValue = 0;
							try
							{
								statusValue = Double.parseDouble(ImageTwoPath);
							}
							catch(NumberFormatException e)
							{
								System.out.println("Job " + item.getJobId() + " ImageTwo Path not a double value.");
								Log.info("Job " + item.getJobId() + " ImageTwo Path not a double value.");
								e.printStackTrace();
							}
							/*String[] tokens = ImageTwoPath.split("\\.");
	               			 int cnt = tokens.length;
	               			 ImageTwoPath=tokens[0]+ "." + tokens[cnt-1];*/
							/*Path p=Paths.get(ImageTwoPath);
							ImageTwoPath=p.getFileName().toString();*/
							ImageTwoPath=ImageTwoPath.substring((ImageTwoPath.lastIndexOf(File.separator))+1);
							System.out.println("ImagTwoPath" + ImageTwoPath);
							Log.info("ImagTwoPath" + ImageTwoPath);
			...
			{
				Log.info("Error reading file " + filePath1);
				e.printStackTrace();
			}	
		...
		if(dirPath != null && dirPath.isDirectory())
		{
			...
			{					
				...
				for(int i = 0; i < count; i++)
				{
					...
					File file  = new File(dirPath, files[i]);
					System.out.println("$$$$$$$$$$$$$$$$$$ Data file: " + file.getPath());
					Log.info("$$$$$$$$$$$$$$$$$$ Data file: " + file.getPath());
					//If an output file exists add it to the list of output files
					if(file.exists())
					{	
						Log.info("FileName: " + fileName);
						Log.info("ImagTwoPath: " + ImageTwoPath);
						if (fileName.equals(ImageOnePath) || fileName.equals(ImageTwoPath))
						{
							String filePath = outputFilesDir + fileName;
							String[] fileData = new String[3];
							fileData[0] = fileName;
							Log.info("Found File Name " + fileData[0]);
							fileData[1] = rootPathForDownload + constants.getString("Data") + File.separator + fileName;
							Log.info("Found File Root Path " + fileData[1]);
							fileData[2] = filePath;
							Log.info("Found File Path " + fileData[2]);
							outputFiles.put(key, fileData);
							key++;
						}
	...
	public JobItem getJobData(File file, String fileName, String filePath, JobItem item)
	{
		...
		try
		{
			...
			if(matfilereader.getMLArray("JobState") != null)
			{
				...
				{
					MLChar jobStatus = (MLChar) mlStructure.getField("MatchingScore");
					String statusStr = jobStatus.contentToString();
					String MatchingScore=null;
					if(statusStr.length() > 0)
					{
						MatchingScore = parseString(statusStr);
						double statusValue = 0;
						try
						{
							statusValue = Double.parseDouble(MatchingScore);
						}
						catch(NumberFormatException e)
						{
							System.out.println("Job " + item.getJobId() + " Matching Score not a double value.");
							Log.info("Job " + item.getJobId() + " Matching Score not a double value.");
							//e.printStackTrace();
						}
					/*	if(statusValue > 0)
						{
					...
					item.setMatchingScore(MatchingScore);
					System.out.println("MatchingScore: " + MatchingScore);
				}
