...
public class JobHistoryViewGwtImpl extends DetailViewGwtImpl implements JobHistoryView { 
	...
	//private ListDataProvider<JobItem> dataProvider;
	//private int counter;
	...
	public CellTable<JobItem> onInitialize()
	{
	  ...
	  //  dataProvider=new ListDataProvider<JobItem>();
	...
	private class MyDataProvider extends AsyncDataProvider<JobItem> 
	 {
		 protected void onRangeChanged(HasData<JobItem> display)
			...
			//	Log.info("@@@Start: " + start + " End: " + (start + length) + " Date: " + new Date() + " user: " + ((ClientContext) RPCClientContext.get()).getCurrentUser().getUsername());
				//service.getPage(start, start + length, !sortList.get(0).isAscending(), "All", currentUser.getUsername(), "plbp", tab, callback);
				service.getPage(start, start + length, !sortList.get(0).isAscending(), "All",currentUser.getUsername(), "IrisCloud", tab, callback);
		    }
	 }
	  ...
	  {	  
	   ...
	   Column<JobItem, String> id = new Column<JobItem, String>(new TextCell()) 
	    {	    
	      ...
	      {
	    	//  return Integer.toString(((List<JobItem>) dataProvider).indexOf(object))+1;
	     return new Integer(object.getSerialNumber()).toString();
	      }
	    };	   
	/*	TextColumn<JobItem> numColumn = new TextColumn<JobItem>() {
	   ...
	   table.addColumn(id, "Serial #");
		//table.addColumn(numColumn, "JobId #");
	   table.setColumnWidth(id, 25, Unit.PCT);
	   // table.setColumnWidth(numColumn, 25, Unit.PCT);
	    ...
	    table.setColumnWidth(dateColumn, 25, Unit.PCT);	   
	    ...
	    Column<JobItem, String> scoreColumn = new Column<JobItem, String>(new TextCell())
	    	    {
	    	    	public String getValue(JobItem object)
	    	    	{
	    	    		
						if(object.getStatus()=="Running")
						{
	    	    			String cal="Score being calculated";
	    	    			return cal;
						}
						else
	    	    		{
							return object.getMatchingScore();
						}
	    	    	}
	    	    };
	    	    table.addColumn(scoreColumn, "Matching Score");
	    	    table.setColumnWidth(scoreColumn, 25, Unit.PCT); 
	  }
