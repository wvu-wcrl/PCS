...
import com.googlecode.mgwt.examples.showcase.server.job.RandomProjection;
...
public class EnrollmentServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    {            
		...
		try {
    		...
    		{
	        	jobUsername = formItems.get("admin");
	        	MasterKeyImpl keyImpl = new MasterKeyImpl();
	        	passwordHash = keyImpl.getHashKey();
	        	jobDir = constants.getString("path") + formItems.get("admin") + File.separator + constants.getString("projects") + File.separator + formItems.get("project") + File.separator + constants.getString("JobIn") + File.separator + jobFileName;
	        	tempDirPath = constants.getString("path") + formItems.get("admin") + File.separator + constants.getString("projects") + File.separator + formItems.get("project") + File.separator + constants.getString("Temp");    		}
	        else
	        {
	        	jobUsername = formItems.get("username");
	        	jobDir = constants.getString("path") + formItems.get("username") + File.separator + constants.getString("projects") + File.separator + formItems.get("project") + File.separator + constants.getString("JobIn") + File.separator + jobFileName;
	        	tempDirPath = constants.getString("path") + formItems.get("username") + File.separator + constants.getString("projects") + File.separator + formItems.get("project") + File.separator + constants.getString("Temp");
	        }
	        ...
	        int exitValue = 0;
       	 	...
       	 	File tempJobFile  = new File(tempDirPath, jobFileName);
       	 	boolean fileGenerated = generateVerificationModelJobFile(tempJobFile, passwordHash, jobDir);
  ...
  private boolean generateVerificationModelJobFile(File jobFile, String passwordHash, String jobDirPath)
  {
 	 ...
 	 try 
 	 {
		...
		fileGenerated = randomProjection.getRandomProjection(jobFile.getPath(), jobDirPath, passwordHash);
