classdef AWGN < handle
 ...
 %   properties
    ...
    methods
        function obj = AWGN( ModulationObj, EsNo,ModulatedSignal )
...
%         end
        ...
        function RecievedSignal = PassToChannel(obj,ModulatedSignal,EsNo)
            obj.ModulatedSignal=ModulatedSignal;
            Noise = sqrt(obj.Variance) * randn(size(obj.ModulationObj.SignalSet,1) , size(obj.ModulatedSignal,2));
            RecievedSignal = Noise + obj.ModulatedSignal;
            obj.RecievedSignal = RecievedSignal;
        end
    end
