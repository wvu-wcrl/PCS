 ...
 classdef p2p < Channel
  ...
  properties
    ...
    methods
        function obj = p2p( ModulationObj, EsN0 )
        ...
        function SymbolLikelihood = ChannelUse(obj, SymbolIndex, EsN0)
            ...
            if ( ModulationObj.ChannelType==AWGN)
            C=AWGN(ModulationObj,EsNo,ModulatedSignal);
            else
                if(ModulationObj.ChannelType==Fading)
            C=Fading(ModulationObj,EsNo); 
            end
            
            end
            
            RecievedSignal = C.PassToChannel(ModulationObj, EsNo,ModulatedSignal);
