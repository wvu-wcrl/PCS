...
public class GetSubscribedDataProjectListImpl extends RemoteServiceServlet implements GetSubscribedDataProjectListService {
	...
	public ProjectItems getProjects(int userId)
	{
		...
		try
		{
			...
			cs = connection.getConnection().prepareCall("{ call GetUserDataProjects(?) }");
			cs.setInt(1, userId);
