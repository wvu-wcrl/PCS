...
import com.wcrl.web.cml.client.jobService.GetUserProjectDataFilesListServiceAsync;
...
public class UploadJobandDataFiles_WithSelectionOfDataFiles extends Composite implements ClickHandler, ChangeHandler 
{
	...
	private RadioButton rdUpload;
	private FileUpload dataUpload;
	private ListBox lbDataFiles;
	private HorizontalPanel dataFileListPanel;
    ...
    public UploadJobandDataFiles_WithSelectionOfDataFiles(int tab) 
    {    
    ...
    {    	
    	//dataPanel.setTitle("Data file");
    	dataPanel.setSpacing(5);   	
    	dataFileListPanel.add(rdList);
    	dataFileListPanel.add(lbDataFiles);
    	    	
    	dataFileUploadPanel.add(rdUpload);
    	dataFileUploadPanel.add(dataUpload);
    	
    	setSelectionDataFileList();
    	rdUpload.setValue(true);
    	
    	dataPanel.add(dataFileListPanel);
	...
	{	
		...
		dataFileListPanel = new HorizontalPanel();
    	...
    	vPanel.add(hlBack);
    	rdList = new RadioButton("group", "Select:");
    	rdUpload  = new RadioButton("group", "Upload:");
    	dataUpload = new FileUpload();
    	lbDataFiles = new ListBox();
		...
		else
		{
			...
			rdList.addClickHandler(this);
			rdUpload.addClickHandler(this);
	        ...
	        form.addSubmitHandler(new FormPanel.SubmitHandler() 
	        {
	        	public void onSubmit(SubmitEvent event) 
	        	{
	        		...
	        		if(ckDataFile.getValue())
	        		{
	        			if(rdList.getValue())
		        		{
	        				if(lbDataFiles.getItemCount() > 0)
	        				{
	        					dataFileName = lbDataFiles.getItemText(lbDataFiles.getSelectedIndex()).trim();
	        				}	
	        				else
	        				{
	        					txtWarnings.setText("");
		        		...
		        		if(rdUpload.getValue())
		        		{
		        			...
		        			if(fileName.length() == 0)
		        			{
		        				txtWarnings.setText("");
			        			txtWarningDesc.setText("");
			        			txtWarnings.setHTML("*Please choose/upload a data file.");
			        			event.cancel();
		        			}
		        			else
		        			{
