classdef HEX < CreateModulation
    ...
    methods
        function obj = HEX(SignalProb)
            % Calling Syntax: obj = HEX( [SignalProb] )
            % (ONLY 16-)HEX modulation constructor.
            Order = 16;
            if( nargin<1 || isempty(SignalProb) )
                SignalProb = [];
            elseif ( length(SignalProb) ~= 16 )
                error('HEX:InvalidSignalProb', 'The 1-by-16 input vector SignalProb should contain signal RELATIVE probabilities for 16-HEX modulation.');
            end
